...
type ListSectionKey = "works" | "achievements" | "products" | "articles";
type AdminSection = ListSectionKey | "profile";
type ProfileContent = {
  lead: string;
  description: string;
  field: string;
...
function newItem(section: ListSectionKey) {
...
function normalizeContent(content: Partial<SiteContent>): SiteContent {
  ...
  const incomingProfile = content.profile && typeof content.profile === "object" ? content.profile : defaultProfile;
...
export default function AdminPanel() {
  ...
  const updateProfile = (key: keyof ProfileContent, value: string | string[]) => {
    setContent((current) => ({ ...current, profile: { ...current.profile, [key]: value } }));
  };

  const updateItem = (section: ListSectionKey, index: number, key: string, value: any) => {
    setContent((current) => ({
      ...current,
  ...
  const deleteItem = (section: ListSectionKey, index: number) => {
    if (!window.confirm("この項目を削除しますか？")) return;
    setContent((current) => normalizeContent({ ...current, [section]: current[section].filter((_, i) => i !== index) }));
  };

  const moveItem = (section: ListSectionKey, index: number, direction: -1 | 1) => {
    setContent((current) => {
      const list = [...current[section]];
      const target = index + direction;
      if (target < 0 || target >= list.length) return current;
      [list[index], list[target]] = [list[target], list[index]];
      return normalizeContent({ ...current, [section]: list });
    });
  };
  ...
  const renderProfileInput = (
    label: string,
    key: keyof ProfileContent,
    options?: { multiline?: boolean; list?: boolean; placeholder?: string },
  ) => {
    const value = options?.list ? listValue(content.profile[key]) : String(content.profile[key] ?? "");
    return <label className="admin-field"><span>{label}</span>{options?.multiline || options?.list ?
      <textarea value={value} rows={options?.multiline ? 4 : 3} placeholder={options?.placeholder} onChange={(e) => updateProfile(key, options?.list ? parseList(e.target.value) : e.target.value)} /> :
      <input value={value} placeholder={options?.placeholder} onChange={(e) => updateProfile(key, e.target.value)} />}</label>;
  };
  ...
  const renderImageUploader = (label: string, section: ListSectionKey, index: number, key: string, multiple = false) => {
  ...
  const renderPdfUploader = (label: string, section: ListSectionKey, index: number, key: string) => {
    const value = content[section][index]?.[key] ?? "";
    const uploadKey = `${section}-${index}-${key}`;
  ...
  const itemHeader = (section: ListSectionKey, item: any, index: number) => <div className="admin-item-head"><strong>{String(index + 1).padStart(2, "0")} / {item.title || "Untitled"}</strong><div className="admin-item-actions"><button type="button" onClick={() => moveItem(section, index, -1)} disabled={index === 0}>↑</button><button type="button" onClick={() => moveItem(section, index, 1)} disabled={index === content[section].length - 1}>↓</button><button type="button" className="danger" onClick={() => deleteItem(section, index)}>削除</button></div></div>;

  const renderProfile = () => <div className="admin-item admin-profile-card"><div className="admin-item-head"><strong>PROFILE / 公開プロフィール</strong></div>
  ...
  const renderWork = (item: any, index: number) => <div className="admin-item" key={item.id || index}>{itemHeader("works", item, index)}
    ...
    <details className="admin-detail" open><summary>PLANNER CASE STUDY</summary>{renderInput("PLANNER POINT（この作品で見てほしい設計判断）", "works", index, "plannerPoint", { multiline: true })}<div className="admin-case-grid">{renderInput("01 / 課題", "works", index, "problem", { multiline: true })}{renderInput("02 / 狙い", "works", index, "intent", { multiline: true })}{renderInput("03 / 判断・仕様", "works", index, "decision", { multiline: true })}{renderInput("04 / 結果・学び", "works", index, "result", { multiline: true })}</div>{renderInput("自分の貢献", "works", index, "contribution", { multiline: true })}{renderInput("その他の工夫", "works", index, "challenge", { multiline: true })}{renderInput("ポイント（1行1項目）", "works", index, "highlights", { list: true })}</details>
  ...
  const renderAchievement = (item: any, index: number) => <div className="admin-item" key={item.id || index}>{itemHeader("achievements", item, index)}<div className="admin-grid">{renderInput("タイトル", "achievements", index, "title", { primary: true })}{renderInput("年 / 期間", "achievements", index, "year", { placeholder: "2026 / 2024–2025" })}{renderInput("カテゴリ", "achievements", index, "category", { placeholder: "AWARD / LEADERSHIP / CONTEST" })}{renderInput("強調ラベル", "achievements", index, "highlight", { placeholder: "最優秀賞 / 優勝 / 約3倍" })}{renderInput("主催・所属", "achievements", index, "organization")}{renderInput("関連URL", "achievements", index, "url", { placeholder: "https://..." })}</div>{renderInput("説明", "achievements", index, "description", { multiline: true })}</div>;
