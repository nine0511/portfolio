...
type SectionKey = "works" | "achievements" | "products" | "articles";
type SiteContent = { works: any[]; achievements: any[]; products: any[]; articles: any[] };

const emptyContent: SiteContent = { works: [], achievements: [], products: [], articles: [] };
...
function normalizeContent(content: Partial<SiteContent>): SiteContent {
  ...
  return {
    works: works.map((item, index) => ({ ...item, number: String(index + 1).padStart(2, "0") })),
    achievements,
    products,
    articles: articles.map((item, index) => ({ ...item, number: String(index + 1).padStart(2, "0") })),
  };
}
...
export default function AdminPanel() {
  ...
  const addItem = () => {
    setContent((current) => normalizeContent({ ...current, [activeSection]: [newItem(activeSection), ...current[activeSection]] }));
  ...
  const deleteItem = (index: number) => {
    ...
    setContent((current) => normalizeContent({ ...current, [activeSection]: current[activeSection].filter((_, i) => i !== index) }));
  };
  ...
  const moveItem = (index: number, direction: -1 | 1) => {
    setContent((current) => {
      ...
      return normalizeContent({ ...current, [activeSection]: list });
    });
  };
  ...
  const uploadFiles = async (
    ...
    options: { multiple?: boolean; pdf?: boolean } = {},
  ) => {
    ...
    if (invalid) {
      setMessage(pdf ? "PDFファイルのみアップロードできます。" : `${invalid.name}: PNG / JPEG / WebP / GIF のみ対応しています。`);
      return;
    }
    const limit = pdf ? maxPdfSize : maxImageSize;
    const oversized = files.find((file) => file.size > limit);
    if (oversized) {
      setMessage(`${oversized.name}: 1ファイル${pdf ? "12" : "4"}MB以下にしてください。`);
      return;
    }
      ...
      <label className="admin-field">
        ...
        {options?.multiline || options?.list ? (
          <textarea
            ...
            onChange={(e) => updateItem(section, index, key, options?.list ? parseList(e.target.value) : e.target.value)}
          />
        ) : (
          <input
            value={value}
            ...
            placeholder={options?.placeholder}
            onChange={(e) => updateItem(section, index, key, e.target.value)}
          />
        )}
      </label>
    );
  ...
  const renderImageUploader = (label: string, section: SectionKey, index: number, key: string, multiple = false) => {
    ...
    return (
      <div className="admin-upload-field">
        <div className="admin-upload-label"><span>{label}</span><small>{multiple ? "複数選択可 / 最大12枚" : "1枚"}・画像4MB以下</small></div>
        <div
          className={`admin-dropzone ${isUploading ? "uploading" : ""}`}
          onDragOver={(e) => e.preventDefault()}
          onDrop={(e) => { e.preventDefault(); if (canSave && !isUploading) uploadFiles(section, index, key, e.dataTransfer.files, { multiple }); }}
        >
          <input
            id={inputId}
            type="file"
            ...
            onChange={(e) => { if (e.target.files) uploadFiles(section, index, key, e.target.files, { multiple }); e.currentTarget.value = ""; }}
          />
        ...
        {paths.length > 0 && (
          <div className={`admin-image-previews ${multiple ? "multiple" : ""}`}>
            {paths.map((path, p) => (
              <figure key={`${path}-${p}`}><img src={path} alt="" /><figcaption>{path}</figcaption><button type="button" onClick={() => updateItem(section, index, key, multiple ? paths.filter((_, i) => i !== p) : "")}>×</button></figure>
            ))}
          </div>
        )}
  ...
  const renderPdfUploader = (label: string, section: SectionKey, index: number, key: string) => {
    ...
    return (
      <div className="admin-pdf-field">
        <div className="admin-upload-label"><span>{label}</span><small>PDF / 12MB以下</small></div>
        <div
          className={`admin-pdf-drop ${isUploading ? "uploading" : ""}`}
          onDragOver={(e) => e.preventDefault()}
          onDrop={(e) => { e.preventDefault(); if (canSave && !isUploading) uploadFiles(section, index, key, e.dataTransfer.files, { pdf: true }); }}
        >
  ...
  const renderWork = (item: any, index: number) => (
    <div className="admin-item" key={item.id || index}>
      ...
      <div className="admin-grid">
        ...
        {renderInput("作品URL", "works", index, "url", { placeholder: "https://..." })}
      </div>
      ...
      <div className="admin-grid">
        ...
        {renderInput("タグ", "works", index, "tags", { list: true })}
        {renderInput("使用技術", "works", index, "tools", { list: true })}
      </div>
      <details className="admin-detail" open>
        <summary>PLANNER CASE STUDY</summary>
        {renderInput("PLANNER POINT（この作品で見てほしい設計判断）", "works", index, "plannerPoint", { multiline: true })}
        <div className="admin-case-grid">
          {renderInput("01 / 課題", "works", index, "problem", { multiline: true })}
          {renderInput("02 / 狙い", "works", index, "intent", { multiline: true })}
          {renderInput("03 / 判断・仕様", "works", index, "decision", { multiline: true })}
          {renderInput("04 / 結果・学び", "works", index, "result", { multiline: true })}
        </div>
        {renderInput("自分の貢献", "works", index, "contribution", { multiline: true })}
        {renderInput("その他の工夫", "works", index, "challenge", { multiline: true })}
  ...
  const renderAchievement = (item: any, index: number) => (
    <div className="admin-item" key={item.id || index}>
      {itemHeader("achievements", item, index)}
      <div className="admin-grid">
        {renderInput("タイトル", "achievements", index, "title", { primary: true })}
        {renderInput("年 / 期間", "achievements", index, "year", { placeholder: "2026 / 2024–2025" })}
        {renderInput("カテゴリ", "achievements", index, "category", { placeholder: "AWARD / LEADERSHIP / CONTEST" })}
        {renderInput("強調ラベル", "achievements", index, "highlight", { placeholder: "最優秀賞 / 優勝 / 約3倍" })}
        {renderInput("主催・所属", "achievements", index, "organization")}
        {renderInput("関連URL", "achievements", index, "url", { placeholder: "https://..." })}
      </div>
      {renderInput("説明", "achievements", index, "description", { multiline: true })}
    </div>
  );
  ...
  const renderProduct = (item: any, index: number) => (
    <div className="admin-item" key={item.id || index}>
      ...
      <div className="admin-grid">
        ...
        {renderInput("種別 / フィルター名", "products", index, "type", { placeholder: "GAME / PLANNING / WEB" })}
  ...
  const renderArticle = (item: any, index: number) => (
    <div className="admin-item" key={item.id || index}>
      ...
      <div className="admin-grid">
        ...
        {renderInput("読了時間", "articles", index, "readTime", { placeholder: "6 MIN READ" })}
        {renderInput("記事URL", "articles", index, "url", { placeholder: "https://..." })}
        {renderInput("タグ", "articles", index, "tags", { list: true })}
      </div>
  ...
  return (
    <>
      ...
      {open && (
        <div className="admin-backdrop" onMouseDown={(e) => { if (e.currentTarget === e.target) setOpen(false); }}>
          <section className="admin-panel" role="dialog" aria-modal="true">
            {!authenticated ? (
              <div className="admin-login">
                ...
                <input type="password" value={password} autoFocus onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") login(); }} />
              ...
              <>
                ...
                <div className="admin-toolbar"><p>追加項目は一番上に作成されます。ACHIEVEでは受賞・活動実績をまとめて編集できます。</p><button onClick={addItem}>＋ 項目を追加</button></div>
                <div className="admin-items">
                  {activeSection === "works" && content.works.map(renderWork)}
                  {activeSection === "achievements" && content.achievements.map(renderAchievement)}
                ...
                <footer className="admin-savebar">
                  <div>{!canSave && <span className="admin-warning">GitHub保存用トークンが未設定です。</span>}{message && <span className="admin-message">{message}</span>}</div>
